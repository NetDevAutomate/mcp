...
# Local imports
from ..server import handle_aws_error, mcp
...
class NetworkFirewallTools:
    ...
    def _check_stateless_rules(self, policy: Dict) -> List[str]:
        """Check for stateless rules in policy."""
        return []

    def _calculate_policy_complexity(self, policy: Dict) -> float:
...
async def monitor_anfw_logs(
    firewall_name: str = Field(
        ..., description=("Network Firewall name. CRITICAL: Must match existing firewall name in region")
    ),
    log_type: Literal["flow", "alert"] = Field(
        "flow", description=('Log type to retrieve. IMPORTANT: Must be either "flow" or "alert"')
    ),
    time_range_minutes: int = Field(
        60, description=("Time window for logs (minutes). IMPORTANT: Must be 1-1440 (max 24h)"), ge=1, le=1440
    ),
    region: str = Field(None, description=("AWS region of the firewall. IMPORTANT: Must be valid region code")),
) -> str:
...
async def analyze_anfw_policy(
    firewall_identifier: str = Field(
        ..., description=("Firewall ARN or name. CRITICAL: Must be valid AWS resource identifier")
    ),
    include_compliance_check: bool = Field(
        True, description=("Include CloudWAN compliance analysis. IMPORTANT: Requires CloudWAN integration")
    ),
    region: str = Field(
        None, description=("AWS region override. IMPORTANT: Use only when firewall is in different region")
    ),
) -> str:
...
async def analyze_five_tuple_flow(
    firewall_identifier: str = Field(
        ..., description=("Firewall ARN/name for analysis. CRITICAL: Must be valid resource identifier")
    ),
    source_ip: str = Field(..., description=("Source IP address. CRITICAL: Must be valid IPv4 format")),
    destination_ip: str = Field(..., description=("Destination IP address. CRITICAL: Must be valid IPv4 format")),
    protocol: str = Field(..., description=("Protocol for flow (tcp/udp/icmp). CRITICAL: Must be valid protocol")),
    source_port: int = Field(..., description=("Source port number. CRITICAL: Must be 1-65535"), ge=1, le=65535),
    destination_port: int = Field(
        ..., description=("Destination port number. CRITICAL: Must be 1-65535"), ge=1, le=65535
    ),
    region: str = Field(None, description=("AWS region override. IMPORTANT: Use only for cross-region firewalls")),
) -> str:
