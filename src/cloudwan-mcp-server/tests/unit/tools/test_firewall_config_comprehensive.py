...
@pytest.mark.parametrize("tool_func, params, success_response", [
    ...
    (simulate_policy_changes,
     {"current_policy": {"rules": []}, "proposed_changes": {"add_rules": ["rule1"]}, "test_traffic": [{"src": "10.0.0.1"}]},
...
@pytest.mark.parametrize("tool_func, invalid_params, error_message", [
    ...
    (monitor_anfw_logs,
     {"firewall_name": "test", "time_range": "invalid"},
     "Invalid time_range format"),
    
    (analyze_anfw_policy,
     {"policy_arn": "invalid-arn"},
     "Invalid policy ARN format"),
    ...
    (parse_suricata_rules,
     {"rule_content": ""},
     "rule_content cannot be empty"),
    
    (simulate_policy_changes,
     {"current_policy": None, "proposed_changes": {}},
     "current_policy is required"),
    
    # Configuration validation errors
    (aws_config_manager,
     {"operation": "invalid_op", "resource_type": "AWS::EC2::Instance"},
     "Invalid operation"),
    ...
    mock_client.execute.return_value = {
        ...
        "analysis": {
            ...
            "default_actions": {
                ...
                "stateless_fragment": ["aws:forward_to_sfe"]
            },
            "complexity_score": 7.5,
            "recommendations": [
                "Consider adding explicit deny rules",
                "Enable logging for all rule groups"
            ]
        }
    }
...
async def test_parse_suricata_rules_complex(mock_client, sample_suricata_rules):
    ...
    result = await parse_suricata_rules(
        mock_client,
        rule_content=sample_suricata_rules,
...
async def test_simulate_policy_changes_comprehensive(mock_client):
    ...
    current_policy = {
        "rules": [
            {"action": "allow", "source": "10.0.0.0/8", "destination": "any", "protocol": "tcp", "port": 443}
        ]
    }
