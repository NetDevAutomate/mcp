...
class TestCloudWANPolicyTools:
    @pytest.mark.parametrize(
        ...
        [
            {
                "version": "2023-01-01",
                "core-network-configuration": {"segments": ["prod", "dev"], "routing-mode": "static"},
            },
            {"version": "2023-02-01", "core-network-configuration": {"segments": [], "routing-mode": "dynamic"}},
        ],
    )
    ...
    async def test_analyze_segment_routes(self, mock_boto_client):
        ...
        mock_client.get_core_network_policy.return_value = {
            "CoreNetworkPolicy": {"PolicyVersion": "1", "Segments": ["prod"]}
        }
        mock_boto_client.return_value = mock_client
