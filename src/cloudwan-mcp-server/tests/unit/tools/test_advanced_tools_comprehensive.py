...
@pytest.mark.parametrize(
    ...
    [
        # Circuit Breaker
        (get_circuit_breaker_status, {"service_name": "test-service"}, {"status": "CLOSED", "failure_threshold": 5}),
        (get_system_resilience_metrics, {"time_window": "1h"}, {"metrics": {"availability": 99.9, "error_rate": 0.1}}),
        # IaC Firewall
        (
            analyze_iac_firewall_policy,
        ...
        (
            simulate_iac_firewall_traffic,
            {
                "policy_json": '{"rules":[{"action":"deny"}]}',
                "traffic_samples": [{"src": "10.0.0.1", "dst": "1.1.1.1"}],
            },
        ...
        # NFG
        (list_network_function_groups, {"region": "us-east-1"}, {"groups": [{"id": "nfg-1", "name": "TestGroup"}]}),
        (analyze_network_function_group, {"group_id": "nfg-1"}, {"analysis": {"functions": 3, "status": "HEALTHY"}}),
    ],
)
...
async def test_resilience_metrics_with_dimensions(mock_client):
    mock_client.execute.return_value = {
        "metrics": {"availability": 99.95, "error_rate": 0.05, "latency_p95": 250},
        "dimensions": {"region": "us-east-1", "service": "network"},
    }
    result = await get_system_resilience_metrics(
        mock_client, time_window="24h", dimensions={"region": "us-east-1", "service": "network"}
    )
...
async def test_analyze_iac_policy_complex(mock_client):
    ...
    mock_client.execute.return_value = {
        "analysis": {"allow_rules": 1, "deny_rules": 1, "conflicts": [], "complexity_score": 5.0}
    }
...
async def test_simulate_iac_firewall_multiple_traffic(mock_client):
    ...
    result = await simulate_iac_firewall_traffic(
        mock_client, policy_json='{"rules":[]}', traffic_samples=traffic_samples, detailed=True
    )
...
async def test_validate_iac_syntax_with_errors(mock_client):
    mock_client.execute.return_value = {
        "is_valid": False,
        "errors": [{"rule_id": "0", "error": "Missing action field"}, {"rule_id": "1", "error": "Invalid CIDR block"}],
    }
...
async def test_nfg_analysis_multiple_functions(mock_client):
    mock_client.execute.return_value = {
        "analysis": {
            "functions": [{"name": "fw", "status": "HEALTHY"}, {"name": "dpi", "status": "DEGRADED"}],
