...
@pytest.mark.parametrize(
    ...
    [
        ...
        # trace_network_path
        (trace_network_path, {"source": "10.0.0.1", "destination": "10.0.0.2"}, {"paths": [{"hops": 5}]}),
        # list_core_networks
        (list_core_networks, {"region": "us-west-2", "status_filter": "AVAILABLE"}, {"core_networks": ["cn-1"]}),
        # get_global_networks
        (get_global_networks, {"region": "global", "filter": "active"}, {"global_networks": ["gn-1"]}),
        # discover_vpcs
        (discover_vpcs, {"region": "us-east-1", "tag_filters": {"Env": "prod"}}, {"vpcs": ["vpc-12345"]}),
        # discover_ip_details
        (discover_ip_details, {"ip_address": "192.168.1.1", "resolve_dns": True}, {"dns_name": "example.com"}),
        # validate_ip_cidr
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (trace_network_path, {"source": "256.0.0.1", "destination": "10.0.0.2"}, "Invalid source IP"),
        # list_core_networks - invalid status
        (list_core_networks, {"status_filter": "INVALID_STATE"}, "Invalid status filter"),
...
async def test_aws_error_handling(mock_client, tool_func, params):
    ...
    mock_client.execute.side_effect = ClientError(
        {"Error": {"Code": "500", "Message": "Internal Failure"}}, "ExecuteOperation"
    )
