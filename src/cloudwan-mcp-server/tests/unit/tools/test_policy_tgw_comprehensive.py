...
@pytest.mark.parametrize("tool_func, params, success_response", [
    ...
    (manage_tgw_routes,
     {"tgw_route_table_id": "tgw-rtb-12345", "operation": "add", "cidr": "10.0.0.0/16", "attachment_id": "tgw-attach-12345"},
...
@pytest.mark.parametrize("tool_func, invalid_params, error_message", [
    ...
    (validate_cloudwan_policy,
     {"policy_document": "invalid-json"},
     "Invalid JSON in policy document"),
    ...
    (get_core_network_policy,
     {"core_network_id": ""},
     "core_network_id cannot be empty"),
    ...
    (analyze_segment_routes,
     {"core_network_id": "core-12345", "segment_name": ""},
     "segment_name cannot be empty"),
    ...
    (analyze_tgw_routes,
     {"tgw_id": "", "route_table_id": "tgw-rtb-12345"},
     "tgw_id cannot be empty"),
...
@pytest.mark.parametrize("tool_func, params", [
    ...
    (manage_tgw_routes, {"tgw_route_table_id": "tgw-rtb-12345", "operation": "add", "cidr": "10.0.0.0/16", "attachment_id": "tgw-attach-12345"}),
...
async def test_validate_complex_policy(mock_client, sample_policy):
    ...
    mock_client.execute.return_value = {
        "is_valid": True,
        "validation_messages": [],
        "policy_version": "2021.12"
    }
    
    result = await validate_cloudwan_policy(
        mock_client,
        policy_document=json.dumps(sample_policy)
    )
...
async def test_analyze_tgw_routes_with_filters(mock_client):
    ...
    for filters in filter_combinations:
        ...
        result = await analyze_tgw_routes(
            mock_client,
            tgw_id="tgw-12345",
...
async def test_get_core_network_change_events_pagination(mock_client):
    ...
    mock_client.execute.return_value = {
        "events": [
            {"timestamp": f"2024-01-01T0{i}:00:00Z", "status": "COMPLETE"}
            for i in range(5)
        ],
        "next_token": "token123"
    }
    ...
    result = await get_core_network_change_events(
        mock_client,
        core_network_id="core-12345",
        change_set_id="cs-12345",
        max_results=5
    )
...
async def test_analyze_tgw_peers_multiple_peerings(mock_client):
    ...
    result = await analyze_tgw_peers(
        mock_client,
        tgw_id="tgw-12345",
        include_all_peers=True
    )
