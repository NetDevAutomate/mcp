...
class TestAWSConfigManager:
    ...
    CACHE_CAPACITY = 10  # Match server.py LRU cache maxsize
    ...
    @pytest.fixture
    def mock_boto3_session_factory(self) -> Generator:
    ...
    @pytest.fixture
    def mock_aws_client_generator(self) -> Generator:
        ...
        with patch('awslabs.cloudwan_mcp_server.server.get_aws_client') as mock_client:
            ...
            def client_side_effect(service, region=None):
                return {'sts': mock_sts, 'ec2': mock_ec2, 'networkmanager': mock_nm}.get(
                    service, Mock()
                )
    ...
    async def test_get_current_configuration_success(
        self, mock_boto3: Mock, mock_aws_client_generator: Generator
    ) -> None:
        ...
        with patch('awslabs.cloudwan_mcp_server.server.aws_config') as mock_aws_config:
            ...
            with patch.dict(
                os.environ,
                {'AWS_PROFILE': 'test-profile', 'AWS_DEFAULT_REGION': 'us-west-2'},
                clear=True,
            ):
    ...
    @pytest.mark.asyncio
    async def test_set_profile_success(self, mock_boto3_session_factory: Generator) -> None:
    ...
    @pytest.mark.asyncio
    async def test_set_region_success(self, mock_boto3_session_factory: Generator) -> None:
    ...
    @pytest.mark.asyncio
    async def test_set_both_success(self, mock_boto3_session_factory: Generator) -> None:
    ...
    async def test_validate_config_success(self, mock_get_aws_client: Generator) -> None:
        ...
        with patch('awslabs.cloudwan_mcp_server.server.get_aws_client') as mock_client:
            ...
            def successful_client(service, region=None):
                mock_svc = Mock()
                if service == 'sts':
                    mock_svc.get_caller_identity.return_value = {
                        'Account': '123456789012',
                        'UserId': 'test-user-id',
                        'Arn': 'arn:aws:iam::123456789012:user/test-user'
                    }
                elif service == 'ec2':
                    mock_svc.describe_regions.return_value = {
                        'Regions': [{'RegionName': f'region-{i}'} for i in range(17)]
                    }
                elif service == 'networkmanager':
                    mock_svc.describe_global_networks.return_value = {
                        'GlobalNetworks': []
                    }
                return mock_svc
            
            mock_client.side_effect = successful_client
            
            with patch.dict(
                os.environ, {'AWS_PROFILE': 'test-profile', 'AWS_DEFAULT_REGION': 'us-east-1'}
            ):
    ...
    async def test_clear_cache_success(
        self, mock_boto3: Mock, mock_get_aws_client: Generator
    ) -> None:
    ...
    async def test_unknown_operation(self) -> None:
        ...
        assert 'supported_operations' in result_data and isinstance(
            result_data['supported_operations'], list
        )
    ...
    async def test_exception_handling(self) -> None:
        ...
        with patch('awslabs.cloudwan_mcp_server.server.aws_config') as mock_aws_config:
            ...
            type(mock_aws_config).profile = property(
            ...
            assert 'Unexpected error' in result_data['error'], (
                f'"Unexpected error" not found in error: {result_data["error"]}'
            )
            assert result_data['error_code'] == 'UnknownError'

    @pytest.mark.asyncio
    async def test_unknown_exception_handling(self) -> None:
        """Test handling of unexpected exceptions in AWSConfigManager."""
        with patch('awslabs.cloudwan_mcp_server.server._create_client') as mock_cache:
            # Mock cache_info to raise an exception at the top level
            mock_cache.cache_info.side_effect = RuntimeError('Unexpected system error')

            result = await aws_config_manager('get_current')
            result_data = json.loads(result)

            assert result_data['success'] is False
            assert 'Unexpected system error' in result_data['error']
            assert result_data['error_code'] == 'UnknownError'
            assert result_data['http_status_code'] == 500

    @pytest.mark.asyncio
    @pytest.mark.skipif(
        not os.environ.get('AWS_PROFILE'),
        reason='Requires live AWS credentials - mark with @pytest.mark.live',
    )
    async def test_validate_config_with_live_credentials(self):
        """Test config validation with actual AWS credentials."""
        pass

    @pytest.mark.asyncio
    async def test_config_persistence_unavailable(self):
        """Test config operations when persistence is unavailable."""
        with patch('awslabs.cloudwan_mcp_server.server.config_persistence', None), \
             patch('boto3.Session') as mock_session, \
             patch('awslabs.cloudwan_mcp_server.server.secure_environment_update', return_value=True):
