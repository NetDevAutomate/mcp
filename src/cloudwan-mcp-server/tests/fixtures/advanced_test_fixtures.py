...
class NetworkTopologyFixtures:
    ...
    @staticmethod
    def generate_enterprise_hub_spoke_topology(
        regions: int = 5, spokes_per_region: int = 10
    ) -> dict[str, Any]:
        ...
        region_names = ['us-east-1', 'us-west-2', 'eu-west-1', 'ap-southeast-1', 'ap-northeast-1'][
            :regions
        ]
            ...
            for spoke_idx in range(spokes_per_region):
                spoke_vpc = {
                    ...
                    'Tags': [
                        ...
                        {
                            'Key': 'Environment',
                            'Value': ['production', 'staging', 'development'][spoke_idx % 3],
                        },
    ...
    def generate_hierarchical_topology(
        levels: int = 3, branches_per_level: int = 4
    ) -> dict[str, Any]:
    ...
    def generate_enterprise_policy(segments: int = 10, rules: int = 50) -> dict[str, Any]:
        ...
        for i in range(segments):
            ...
            if 'production' in segment_name:
                ...
                shared_segments = [
                    s['name'] for s in policy['segments'] if 'shared-services' in s['name']
                ]
            elif 'shared-services' in segment_name:
                # Shared services can be accessed by all
                shared_segments = [
                    s['name'] for s in policy['segments'] if s['name'] != segment_name
                ][:5]
            else:
                ...
                shared_segments = [
                    ...
                    and s['name'].split('-')[0] == segment_name.split('-')[0]
                ][:3]
...
def enterprise_hub_spoke_topology():
    ...
    return NetworkTopologyFixtures.generate_enterprise_hub_spoke_topology(
        regions=5, spokes_per_region=10
    )
