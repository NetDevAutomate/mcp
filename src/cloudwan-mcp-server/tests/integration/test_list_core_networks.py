            ...
            for i, content in enumerate(result.content):
                ...
                try:
                    ...
                    if core_networks:
                        ...
                        logger.info(
                            'ℹ️  No core networks found - this may be expected if CloudWAN is not configured'
                        )
...
async def test_through_mcp_server() -> bool | None:
    ...
    try:
        ...
        request = CallToolRequest(
            method='tools/call', params={'name': 'list_core_networks', 'arguments': {}}
        )
            ...
            if hasattr(actual_result, 'content') and actual_result.content:
                ...
                for i, content in enumerate(actual_result.content):
                    ...
                    try:
                        ...
                        if core_networks:
                            logger.info(
                                f'🎉 MCP SUCCESS: Found {len(core_networks)} core networks!'
                            )
                        else:
