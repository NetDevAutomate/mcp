...
class TestNetworkManagerServiceMocking:
    ...
    async def test_network_manager_list_core_networks_detailed_mock(self) -> None:
        ...
        mock_core_networks = [
            {
                'CoreNetworkId': 'core-network-01234567890abcdef',  # pragma: allowlist secret
                'GlobalNetworkId': 'global-network-01234567890abcdef',  # pragma: allowlist secret
                'State': 'AVAILABLE',
            ...
            {
                'CoreNetworkId': 'core-network-fedcba0987654321',  # pragma: allowlist secret
                'GlobalNetworkId': 'global-network-fedcba0987654321',  # pragma: allowlist secret
                'State': 'UPDATING',
        ...
        with patch('awslabs.cloudwan_mcp_server.server.get_aws_client') as mock_get_client:
            ...
            prod_network = next(
                (n for n in parsed['core_networks'] if n['State'] == 'AVAILABLE'), None
            )
    ...
    async def test_network_manager_core_network_policy_detailed(self) -> None:
        ...
        complex_policy_doc = {
            ...
            'segment-actions': [
                {'action': 'share', 'segment': 'production', 'share-with': ['development']}
            ],
        }
...
class TestEC2ServiceMocking:
    ...
    async def test_ec2_describe_vpcs_comprehensive_mock(self) -> None:
        ...
        with patch('awslabs.cloudwan_mcp_server.server.get_aws_client') as mock_get_client:
            ...
            prod_vpc = next(
                (
                    v
                    for v in parsed['vpcs']
                    if v.get('Tags', [{}])[0].get('Value') == 'Production VPC'
                ),
                None,
            )
    ...
    async def test_ec2_transit_gateway_peering_detailed_mock(self) -> None:
        ...
        mock_peering_attachment = {
            'TransitGatewayAttachmentId': 'tgw-attach-01234567890abcdef',  # pragma: allowlist secret
            'RequesterTgwInfo': {
...
class TestIntegratedServiceMocking:
    ...
    async def test_multi_service_workflow_mock(self) -> None:
        ...
        # Mock EC2 VPCs
        mock_vpcs = [
            {'VpcId': 'vpc-01234567890abcdef0', 'State': 'available', 'CidrBlock': '10.0.0.0/16'}
        ]
