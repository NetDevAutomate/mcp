...
class TestListCoreNetworksPagination:
    ...
    async def test_list_core_networks_pagination_1000_items(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=pagination_mock
        ):
    ...
    async def test_describe_global_networks_token_cycling(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=token_cycling_mock
        ):
    ...
    async def test_search_transit_gateway_routes_page_handling(self) -> None:
        ...
        routes_page_1 = [
            {'DestinationCidrBlock': f'10.{i}.0.0/16', 'State': 'active', 'Type': 'propagated'}
            for i in range(100)
        ]

        routes_page_2 = [
            {'DestinationCidrBlock': f'172.16.{i}.0/24', 'State': 'active', 'Type': 'static'}
            for i in range(100)
        ]
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=route_pagination_mock
        ):
            ...
            assert parsed['success'] is True
            assert (
                parsed['analysis']['total_routes'] == 100
            )  # Current implementation uses first page
    ...
    async def test_max_results_parameter_boundary_testing(self) -> None:
        ...
        for max_results in max_results_scenarios:
            with patch('awslabs.cloudwan_mcp_server.server.get_aws_client') as mock_get_client:
                ...
                networks = [
                    {'CoreNetworkId': f'core-network-{i:05d}', 'State': 'AVAILABLE'}
...
class TestRateLimitingScenarios:
    ...
    async def test_adaptive_retry_strategy_validation(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=adaptive_retry_mock
        ):
            result = await list_core_networks()
    ...
    async def test_rate_limiting_with_burst_capacity(self) -> None:
        ...
        class TokenBucketRateLimiter:
            """Minimal token bucket for test purposes."""

            def __init__(self, burst: int, rate: int) -> None:
                self.burst = burst
                self.rate = rate
                self.count = 0
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=burst_capacity_mock
        ):
...
class TestPaginationWithErrorHandling:
    ...
    async def test_mixed_success_error_pagination_flows(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=mixed_response_mock
        ):
            result = await list_core_networks()
    ...
    async def test_pagination_with_concurrent_modifications(self) -> None:
        ...
        with patch(
            ...
            side_effect=concurrent_modification_mock,
        ):
            result = await list_core_networks()
    ...
    async def test_pagination_cache_invalidation(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client',
            side_effect=cache_invalidation_mock,
        ):
            result = await list_core_networks()
...
class TestCrossRegionPaginationChallenges:
    ...
    async def test_cross_region_pagination_token_isolation(self) -> None:
                    ...
                    raise ClientError(
                        {
                            'Error': {
                                ...
                                'ValidRegions': list(region_tokens.keys()),
                            }
                        },
                        'ListCoreNetworks',
                    )
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=region_specific_mock
        ):
            ...
            for region in regions:
                ...
                assert all(
                    region in network['CoreNetworkId'] for network in parsed['core_networks']
                )
    ...
    async def test_multi_account_pagination_scenarios(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=multi_account_mock
        ):
            result = await list_core_networks()
            ...
            assert parsed['total_count'] == 100
            assert all(
                'account1' in network['CoreNetworkId'] for network in parsed['core_networks']
            )
...
class TestPaginationPerformanceBenchmarks:
    ...
    async def test_partial_success_pagination_handling(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=partial_success_mock
        ):
            result = await list_core_networks()
    ...
    async def test_time_bound_pagination_token_expiry(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=time_bound_token_mock
        ):
            result = await list_core_networks()
