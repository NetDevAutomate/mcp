...
try:
    REGEX_DOS_TIMEOUT = int(
        os.getenv('REGEX_DOS_TIMEOUT', '30')
    )  # Timeout for regex DOS prevention tests
except (ValueError, TypeError):
    ...
    async def test_10mb_policy_document_parsing(self) -> None:
            ...
            for az_idx in range(8):  # 8 AZs per region
                for instance_idx in range(5):  # 5 instances per AZ
                    edge_location = {
                        ...
                        'inside-cidr-blocks': [
                            f'169.254.{region_idx}.{az_idx * 32 + instance_idx * 4}/30'
                        ],
                        'tags': {
                    ...
                    massive_policy['core-network-configuration']['edge-locations'].append(
                        edge_location
                    )
        ...
        for segment_idx in range(50000):
            segment = {
                ...
                'deny-filter': [f'192.168.{segment_idx % 256}.0/24']
                if segment_idx % 7 == 0
                else [],
        ...
        assert memory_usage < 500, (
            f'Memory usage {memory_usage:.2f}MB, expected < 500MB (optimized from 1000MB)'
        )
        ...
        print(
            f'Performance: {policy_size_mb:.1f}MB policy parsed in {parsing_time:.2f}s using {memory_usage:.1f}MB'
        )
...
class TestRegexPerformanceAndDOS:
    ...
    async def test_regex_dos_prevention_patterns(self) -> None:
            ...
            'segments': [
                {
                    'name': f'regex-test-{i}',
                    'description': 'a' * 1000
                    + 'b' * 1000,  # Long strings that could cause regex issues
                    'allow-filter': [
        ...
        assert execution_time < REGEX_DOS_TIMEOUT, (
            f'Regex validation took {execution_time:.2f}s, possible DOS'
        )
    ...
    async def test_large_policy_version_diff_analysis(self) -> None:
        ...
        base_policy = {
            ...
            'core-network-configuration': {
                'asn-ranges': ['64512-64555'],
                'edge-locations': [
                    {'location': f'us-east-{i}', 'asn': 64512 + i} for i in range(1, 5)
                ],
            },
            'segments': [
                {
        ...
        for level in range(5):  # 5 levels of hierarchy
            for group_idx in range(10**level):  # Exponential growth
                group = {
                    ...
                    'dependencies': [f'nfg-level-{level - 1}-{group_idx // 10:06d}']
                    if level > 0
                    else [],
                }
...
class TestPolicyCacheInvalidation:
    ...
    async def test_policy_cache_invalidation_performance(self) -> None:
        ...
        cached_policy = {
            ...
            'segments': [
                {'name': f'cached-segment-{i:04d}', 'require-attachment-acceptance': i % 2 == 0}
                for i in range(5000)
            ],
        }
    ...
    async def test_parallel_policy_validation_performance(self) -> None:
        ...
        for variant_idx in range(20):
            variant_policy = {
                ...
                'core-network-configuration': {
                    ...
                    'edge-locations': [
                        {
                            'location': f'us-east-{(variant_idx % 2) + 1}',
                            'asn': 64512 + variant_idx,
                        }
                    ],
                },
                'segments': [
        ...
        assert sequential_time > 0 and parallel_time > 0, (
            'Both methods should take measurable time'
        )
