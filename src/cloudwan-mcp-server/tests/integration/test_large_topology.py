...
class TestLargeVPCDiscoveryPerformance:
    ...
    async def test_vpc_discovery_memory_optimized_streaming(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=streaming_vpc_mock
        ):
...
class TestMassiveRouteTableAnalysis:
    ...
    async def test_route_analysis_with_chunked_processing(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=chunked_route_mock
        ):
            start_time = time.time()
            ...
            with patch('awslabs.cloudwan_mcp_server.server.get_aws_client') as mock_get_client:
                ...
                if batch_attachments:
                    result = await analyze_tgw_peers(
                        batch_attachments[0]['TransitGatewayAttachmentId']
                    )
        ...
        for level1 in range(100):
            for level2 in range(50):
                action = {
                    ...
                    'share-with': [
                        f'segment-{level1:02d}-{k:02d}'
                        for k in range(level2 + 1, min(level2 + 10, 50))
                    ],
                    'conditions': {
                        'nested-level-1': {
                            'nested-level-2': {
                                'nested-level-3': {
                                    'nested-level-4': {
                                        'nested-level-5': [
                                            {
                                                'condition': f'condition-{level1}-{level2}-{j}',
                                                'value': f'value-{j}',
                                            }
                                            for j in range(20)
                                        ]
    ...
    async def test_cidr_block_analysis_large_dataset(self) -> None:
        ...
        for subnet_size in [8, 12, 16, 20, 24, 28]:
            for network in range(0, 256, max(1, 256 // (32 - subnet_size))):
                ...
                cidr_test_cases.append(
                    f'172.{16 + (network // 16)}.{network % 16}.0/{subnet_size}'
                )
        ...
        assert successful_cidr_validations > 1000, (
            f'Expected >1000 CIDR validations, got {successful_cidr_validations}'
        )
    ...
    async def test_complex_network_topology_limits(self) -> None:
        ...
        with patch('awslabs.cloudwan_mcp_server.server.get_aws_client') as mock_get_client:
            ...
            mock_client.describe_global_networks.return_value = {
                'GlobalNetworks': [
                    {'GlobalNetworkId': gn} for gn in topology_data['global_networks']
                ]
            }
            ...
            assert processing_time < 45.0, (
                f'Complex topology processing took {processing_time:.2f}s'
            )
