...
class TestThrottlingExceptionScenarios:
    ...
    async def test_throttling_cascade_across_services(self) -> None:
                ...
                mock_client.list_core_networks.side_effect = ClientError(
                    {
                        'Error': {
                            'Code': 'ThrottlingException',
                            'Message': 'NetworkManager throttled',
                        }
                    },
                    'ListCoreNetworks',
                )
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=throttle_side_effect
        ):
...
class TestCrossServiceErrorPropagation:
    ...
    async def test_networkmanager_to_ec2_error_propagation(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=progressive_failure
        ):
    ...
    async def test_authentication_error_propagation(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=auth_failure_factory
        ):
...
class TestErrorRecoveryPatterns:
    ...
    async def test_retry_after_temporary_failure(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=temporary_failure_mock
        ):
    ...
    async def test_circuit_breaker_pattern_simulation(self) -> None:
        ...
        with patch(
            'awslabs.cloudwan_mcp_server.server.get_aws_client', side_effect=circuit_breaker_mock
        ):
            results = []
                ...
                error = ClientError(
                    {
                        'Error': {
                            ...
                            'Type': 'Client'
                            if scenario['error_code'] != 'InternalFailure'
                            else 'Server',
                        },
                        'ResponseMetadata': {
                            'RequestId': f'req-{scenario["error_code"]}-123',
                            'HTTPStatusCode': 403
                            if scenario['error_code'] == 'AccessDenied'
                            else 400,
                        },
                    },
                    scenario['operation'],
                ...
                mock_client.list_core_networks.side_effect = ClientError(
                    {
                        ...
                        'ResponseMetadata': {
                            'HTTPHeaders': {'content-language': error_scenario['locale']}
                        },
                    },
                    'ListCoreNetworks',
                )
