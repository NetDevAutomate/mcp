...
class TestMCPServerIntegration:
    ...
    def test_all_tools_registered(self) -> None:
        ...
        assert len(registered_tools) == len(expected_tools), (
            f'Expected 16 tools, found {len(registered_tools)}'
        )
...
class TestMCPToolExecution:
    ...
    @pytest.mark.asyncio
    async def test_multiple_tools_concurrent_execution(
        self, mock_get_aws_client, mock_aws_context
    ) -> None:
...
class TestMCPProtocolCompliance:
    ...
    @pytest.mark.parametrize(
        'tool_name', ['trace_network_path', 'list_core_networks', 'get_global_networks']
    )
    @pytest.mark.asyncio
