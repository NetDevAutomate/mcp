...
# Comprehensive test patterns for security validation - ALL TEST DATA ONLY
# pragma: allowlist secret
AWS_TEST_PATTERNS = {
    "access_key": "AKIAIOSFODNN7EXAMPLE",  # AWS docs standard test access key
    "secret_key": "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY",  # AWS docs standard test secret
    "session_token": "AQoEXAMPLEH4aoAH0gNCAPyJxz4BlCFFxWNE1OPTgk5TthT+FvwqnKwRcOIfrRh3c/LTo6UDdyJwOOvEVPvLXCrrrUtdnniCEXAMPLE/IvU1dYUg2RVAJBanLiHb4IgRmpRV3zrkuWJOgQs8IZZaIv2BXIa2R4Olgk",  # Test session token
    "high_entropy": "aGVsbG8gd29ybGQgdGhpcyBpcyBhIHZlcnkgbG9uZyB0ZXN0IHN0cmluZw==",  # Base64 test data
}


class TestSecurityCompliance:
    """Comprehensive security compliance testing."""

    def test_all_credential_patterns_sanitized(self):
        """Test that all credential patterns are properly sanitized."""
        for pattern_type, pattern_value in AWS_TEST_PATTERNS.items():
            # pragma: allowlist secret
            test_message = f"Error occurred with {pattern_type}: {pattern_value}"

            # ... existing code ...

    def test_error_message_security_scrubbing(self):
        """Test comprehensive error message security scrubbing."""
        # pragma: allowlist secret
        dangerous_error = f"AWS Error: Invalid access key {AWS_TEST_PATTERNS['access_key']} for secret {AWS_TEST_PATTERNS['secret_key']}"
