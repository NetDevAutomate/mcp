            ...
            with patch('awslabs.cloudwan_mcp_server.server.get_aws_client') as mock_get_client:
                ...
                if expected_success:
                    mock_client.list_core_networks.return_value = {
                        'CoreNetworks': [
                            {'CoreNetworkId': test_case['resource_id'], 'State': 'AVAILABLE'}
                        ]
                    }
                else:
                    mock_client.list_core_networks.side_effect = ClientError(
...
class TestPolicyDocumentSecurity:
    ...
    async def test_malicious_policy_structure_detection(self) -> None:
        ...
        malicious_policies = [
            ...
            {
                'version': '2021.12',
                'core-network-configuration': self._create_deeply_nested_structure(10),
            },
    ...
    async def test_policy_size_limits(self) -> None:
        ...
        for size_name, target_size in policy_sizes:
            ...
            while current_size < target_size and segment_count < 10000:
                segment = {
                    k: v.format(segment_count) if isinstance(v, str) else v
...
class TestCredentialHandlingSecurity:
    ...
    async def test_credential_exposure_prevention(self) -> None:
        ...
        credential_patterns = [
            'AKIA1234567890ABCDEF',  # AWS Access Key  # pragma: allowlist secret
            'wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY',  # AWS Secret Key  # pragma: allowlist secret
    ...
    def _assert_credential_not_in_text(self, credential_patterns, response_text) -> None:
        ...
        for credential in credential_patterns:
            assert credential not in response_text, (
                f'Credential {credential[:8]}*** found in response'
            )

    def _assert_no_credential_in_error_field(
        self, credential_patterns, response_text, parsed
    ) -> None:
        ...
        if 'AWS_ACCESS_KEY_ID' in response_text:
            error_field = (
                parsed.get('current_configuration', {}).get('identity', {}).get('error', '')
            )
            assert 'AWS_ACCESS_KEY_ID' in error_field, (
                "'AWS_ACCESS_KEY_ID' found outside of error field"
            )
    ...
    async def test_temporary_credential_handling(self) -> None:
        ...
        temporary_creds = {
            'AccessKeyId': 'ASIATEMP1234567890',  # pragma: allowlist secret
            'SecretAccessKey': 'temp_secret_key_example',  # pragma: allowlist secret
            'SessionToken': 'temp_session_token_very_long_example_1234567890',
            'Expiration': datetime.now(UTC).isoformat(),
        }

        with patch('boto3.Session') as mock_session:
            mock_session.return_value.get_credentials.return_value.token = temporary_creds[
                'SessionToken'
            ]
    ...
    async def test_role_assumption_security(self) -> None:
        ...
        with patch('awslabs.cloudwan_mcp_server.server.boto3.Session') as mock_session:
            ...
            mock_sts.assume_role.return_value = {
                'Credentials': {
                    'AccessKeyId': 'ASIA1234567890ABCDEF',  # pragma: allowlist secret
                    'SecretAccessKey': 'assumed_role_secret_key',  # pragma: allowlist secret
    ...
    async def test_credential_validation_security(self) -> None:
        ...
        invalid_credentials = [
            ...
            ('AKIA1234567890ABCDEF', ''),  # Missing secret key  # pragma: allowlist secret
            (
                '',
                'wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY',  # pragma: allowlist secret
            ),  # Missing access key  # pragma: allowlist secret
        ]
