...
class TestLiveCoreNetworkOperations:
    ...
    @pytest.mark.live
    @pytest.mark.asyncio
    async def test_live_core_network_listing(
        self, live_aws_credentials, live_test_resources
    ) -> None:
    ...
    @pytest.mark.live
    @pytest.mark.asyncio
    async def test_live_global_network_discovery(
        self, live_aws_credentials, live_test_resources
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_live_multi_region_consistency(
        self, live_aws_credentials, live_test_resources
    ) -> None:
        ...
        for region, results in region_results.items():
            ...
            if results['global_networks']['success']:
                global_network_ids = [
                    gn['GlobalNetworkId'] for gn in results['global_networks']['global_networks']
                ]

                # Store for cross-region comparison on the test instance
                if not hasattr(self, '_global_networks_ref'):
                    self._global_networks_ref = global_network_ids
                else:
                    # Global networks should be the same across regions
                    ref_ids = set(self._global_networks_ref)
...
class TestLivePerformanceMetrics:
    ...
    @pytest.mark.slowtest
    @pytest.mark.asyncio
    async def test_live_api_response_times(
        self, live_aws_credentials, live_test_resources
    ) -> None:
