...
# Test patterns for credential sanitization validation - NOT PRODUCTION SECRETS
# pragma: allowlist secret
TEST_AWS_ACCESS_KEY = "AKIAIOSFODNN7EXAMPLE"  # Standard AWS test access key pattern
# pragma: allowlist secret
TEST_AWS_SECRET_KEY = "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"  # Standard AWS test secret pattern
# pragma: allowlist secret
TEST_SESSION_TOKEN = "IQoJb3JpZ2luX2VjEHYaCXVzLXdlc3QtMiJHMEUCIQDTGhHUqHXwJ"  # Base64 test pattern for session tokens
# pragma: allowlist secret
TEST_API_KEY = "sk-1234567890abcdef1234567890abcdef12345678"  # High entropy test API key pattern
...
def test_sanitize_aws_credentials_comprehensive():
    """Comprehensive test of AWS credential sanitization."""
    # pragma: allowlist secret
    test_message = f"Failed to authenticate with key {TEST_AWS_ACCESS_KEY} and secret {TEST_AWS_SECRET_KEY}"

    sanitized = sanitize_error_message(test_message)
    assert TEST_AWS_ACCESS_KEY not in sanitized
    assert TEST_AWS_SECRET_KEY not in sanitized
    assert "[CREDENTIAL_REDACTED]" in sanitized


def test_high_entropy_string_detection():
    """Test detection of high entropy strings in error messages."""
    # pragma: allowlist secret
    high_entropy_string = "aB3dE5fG7hI9jK1lM2nO4pQ6rS8tU0vW"  # Test high entropy pattern

    test_message = f"API key validation failed: {high_entropy_string}"
    sanitized = sanitize_error_message(test_message)
    assert high_entropy_string not in sanitized
    assert "[CREDENTIAL_REDACTED]" in sanitized
